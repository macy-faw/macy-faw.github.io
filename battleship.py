__author__ = 'Macy Faw, macyfaw@ad.unc.edu, Onyen = macyfaw'
import random
...
class Player:
    ...
    def display_statistics(self):
        print(f"{self.player_name}'s statistics: You have attacked {self.attacks} times, with {self.hits} hits and {self.misses} misses.")
...
# Creates a new class that runs if the player chooses 1 player, so the second player is the computer.
class ComputerPlayer(Player):
    # Initializes the computer player with a name, a blank board, and a fleet of five boats.
    def __init__(self, player_name):
        super().__init__(player_name)
        self.board = Board()
        self.fleet = [Boat("Aircraft Carrier", 5), Boat("Battleship", 4), Boat("Submarine", 3),
                      Boat("Destroyer", 3), Boat("Patrol Boat", 2)]
        self.opponent = None
        self.attacks = 0
        self.hits = 0
        self.misses = 0
    ...
    # Position the fleet, one boat at a time, but by referencing the function that will randomly place the boats,
    # like a computer would do.
    def position_fleet(self):
        # Position the boats randomly.
        for boat in self.fleet:
            self.position_boat_randomly(boat)

    # This new function randomly assigns each boat in this
    def position_boat_randomly(self, boat):
        # Randomly chooses the boat positioning to be either vertical or horizontal
        orientation = random.choice(["h", "v"])
        # Following two lines set x and y to random places on the board, between 0 and 9
        x = random.randint(0, 9)
        y = random.randint(0, 9)
        # Sets the orientation and position of the boat
        boat.set_orientation(orientation)
        boat.set_position(x, y)
        while not self.board.add_boat(boat):
            # If the board refuses to add the boat because of overlap, choose a new random position.
            orientation = random.choice(["h", "v"])
            x = random.randint(0, 9)
            y = random.randint(0, 9)
            boat.set_orientation(orientation)
    ...
    def take_turn(self):
        # Generate a random attack position.
        self.display_statistics()
        x = random.randint(0, 9)
        y = random.randint(0, 9)
        ...
        hit_flag = self.opponent.board.attack(x, y)
        if hit_flag:
            # Prints if the computer hit. Increases the number of hits that will print.
            print("The computer attacked ({}, {})".format(x, y), "and hit a boat!")
            self.hits += 1
        else:
            # Prints if computer misses. Increases misses variable so number would increase when printed
            print("The computer attacked ({}, {}).".format(x, y), "and missed.")
            self.misses += 1

        # Increases the number of attacks, also for when it prints
        self.attacks += 1
...
# Create the game instance with one player and the computer
game = BattleshipGame(num_of_players=1)
